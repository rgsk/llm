...
class GPTConfig:
    ...
    n_head: int  # nh
    n_layer: int
    max_seq: int  # max block_size that can be handled during inference
...
class GPT(nn.Module):
    ...
    def grow_rope(self) -> None:
        """Assigning to a registered name keeps it a buffer."""
        cos, sin = rope_tables(self.max_seq, self.head_size)
        self.rope_cos = cos.to(self.rope_cos)  # match device and dtype
        self.rope_sin = sin.to(self.rope_sin)
    ...
    def forward(
        ...
        assert T_past + T <= self.max_seq
        if T_past + T > self.rope_cos.size(0):
            self.grow_rope()
...
def generate(
    ...
    for _ in range(max_new_tokens):
        if use_cache:
            ...
            logits = model(idx[:, -model.max_seq :])
        logits = logits[:, -1, :]  # [B, V]
...
def run():
    ...
    sample = tok.decode(
        generate(
            model,
            prompt,
            max_new_tokens=model.max_seq - prompt.size(1) + 1,
            use_cache=True,
        )[0].tolist()
    )
    print(sample)
...
def test():
    for kind in get_args(Attention):
        ...
        for new_tokens in (model.max_seq - prompt.size(1), model.block_size + 100):
            ...
            with torch.no_grad():
                ids = generate(model, prompt, max_new_tokens=new_tokens)
                full = model(ids)
                for chunk in (1, 2, 5):
                    caches, step = None, []
                    for t in range(0, ids.size(1), chunk):
                        lg, caches = model(
                            ids[:, t : t + chunk], caches, use_cache=True
                        )
                        step.append(lg)
            ...
            for seed in range(4):
                ...
                a = generate(model, prompt, max_new_tokens=new_tokens + 1)
                torch.manual_seed(seed)
                b = generate(
                    model, prompt, max_new_tokens=new_tokens + 1, use_cache=True
                )
                assert torch.equal(a, b), (
                    f"{kind} seed {seed}: cached generate diverged"
                )
